...
function 得到所有的内容明细页面($pages)     {
  ...
  if(is_array($可以随机抽取使用的页面数据[5])) {
    $可以随机抽取使用的页面数据[5] = array_merge($可以随机抽取使用的页面数据[5], $可以随机抽取使用的页面数据[5], $可以随机抽取使用的页面数据[5]);
  }
  //额外复制两份模板的数据出来,为随后的模板匹配创造更多的匹配项
  if(is_array($可以随机抽取使用的页面数据[7])) {
    $可以随机抽取使用的页面数据[7] = array_merge($可以随机抽取使用的页面数据[7], $可以随机抽取使用的页面数据[7], $可以随机抽取使用的页面数据[7]);
  }
  //额外复制两份模板的数据出来,为随后的模板匹配创造更多的匹配项
  if(is_array($可以随机抽取使用的页面数据[9])) {
    $可以随机抽取使用的页面数据[9] = array_merge($可以随机抽取使用的页面数据[9], $可以随机抽取使用的页面数据[9], $可以随机抽取使用的页面数据[9]);
  }
